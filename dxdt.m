...
speed = norm(ipl(4:6));
dynPress = .5 * rho * (speed^2);
alpha = atan(ipl(6)/ipl(4));
beta = asin(ipl(5)/speed);
vInertial = inv(transformItoV(ipl))*ipl(4:6);
...
alphaDot = (wDot*ipl(4)-uDot*ipl(6))/(ipl(4)^2+ipl(6)^2);
...
qDot = (moment(2)-(Ixx-Izz)*ipl(9)*ipl(7)-Ixz*(ipl(7)^2-ipl(9)^2))/Iyy;
...
psiDot = (1/cos(ipl(11)))*(ipl(8)*sin(ipl(10))+ipl(9)*cos(ipl(10)));
